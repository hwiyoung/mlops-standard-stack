...
업로드된 데이터를 **STAC API** 기반 지도 위에서 직관적으로 관리하고 확인합니다.

### 스텝 1: 데이터 인덱싱 (Indexing)
- **지도 브라우저** 탭 우측의 `📊 신규 데이터 인덱싱` 섹션을 엽니다.
- `버킷`과 `Prefix`를 입력하고 `🔄 인덱싱 실행`을 클릭합니다.
- 인덱서가 GPS 정보를 추출하여 **STAC API**에 등록합니다.

### 스텝 2: STAC 컬렉션 필터
- **📁 컬렉션** 드롭다운에서 데이터 유형을 선택합니다:
    - `전체`: 모든 데이터
    - `drone-photos`: 드론/항공 사진
    - `orthoimages`: 정사영상(GeoTIFF)
